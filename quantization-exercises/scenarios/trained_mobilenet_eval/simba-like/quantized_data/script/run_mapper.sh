...
ARCH="8bit_int_changed"        # choices are: "16bit_int", "8bit_int_same", "8bit_int_changed"
...
# Simba-like (only 1 chip from original simba chiplet structure, architecture similar to NVDLA), configured on 45nm technology using either 16-bit int data or 8bit int data and word widths, the architecture contains 16 PEs
...
for problem in ../prob/$NETWORK/*/*.yaml; do
    start_time=`date +%s`
    timeloop-mapper ../arch/$ARCH/simba_like.yaml ../arch/$ARCH/components/*.yaml ../constraints/*.yaml $mapper $problem -o ../outputs
    end_time=`date +%s`
