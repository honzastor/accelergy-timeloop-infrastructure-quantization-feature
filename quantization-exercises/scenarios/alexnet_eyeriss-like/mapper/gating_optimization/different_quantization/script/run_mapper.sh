...
HEURISTIC="exhaustive"  # choices are: "exhaustive", "linear", "random", "hybrid"
...
mkdir -p ../outputs
for mapper in ../mapper/$THREADS/$HEURISTIC/*.yaml; do
    for problem in ../prob/$NETWORK/*.yaml; do
        start_time=`date +%s`
        timeloop-mapper ../arch/eyeriss_like.yaml ../arch/components/*.yaml ../constraints/*.yaml ../sparse_opt/*.yaml $mapper $problem -o ../outputs
        end_time=`date +%s`
